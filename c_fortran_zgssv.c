//
// Modified by K. Key for interfacing with MARE2D codes.
//
...
typedef long fptr;  /* Use this for 64 bit LP64 machines (OS X 10.5) */
...
/*----------------------------------------------------------------*/
/* Factor a matrix from scratch                                   */
/*----------------------------------------------------------------*/
    if ( *iopt == 1 ) { /* LU decomposition */

		/* Set the default input options. */
		set_default_options(&options); 
		/* KWK: setting these options for 2D MT/CSEM */
		options.SymmetricMode   = YES;
		options.ColPerm         = MMD_AT_PLUS_A; 
		options.DiagPivotThresh = 0.00;   
		...
		zCreate_CompCol_Matrix(&A, *n, *n, *nnz, values, colind, rowptr,
				   SLU_NC, SLU_Z, SLU_GE);
				   
	    /* Allocate memory for LU factors and permutations */			   
		...
		*/    	
		...
		/* Permute columns of A, yielding AC */
...
//		printf("%d\   %d\   %5.3f\n ",
//		   *ldb, *nnz, mem_usage.total_needed/1e6);
...
/*----------------------------------------------------------------*/
/* Triangular solve using existing LU factors                     */
/*----------------------------------------------------------------*/
...
/*----------------------------------------------------------------*/
/* TRANSPOSE Triangular solve using existing LU factors                     */
/*----------------------------------------------------------------*/
    } else if ( *iopt == 22 ) { /* Triangular solve */
    trans = TRANS; 
	/* Initialize the statistics variables. */
	StatInit(&stat);
